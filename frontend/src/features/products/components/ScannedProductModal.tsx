    ...
    return (
        <Modal title="" onClose={onClose} size="lg">
            <div className="space-y-6 -mt-4">
                ...
                <div className="relative bg-primary/10 rounded-3xl p-6 border border-primary/20 overflow-hidden">
                    {/* Background Pattern */}
                    ...
                    <div className="relative flex gap-5 items-start">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-2 mb-2 flex-wrap">
                                ...
                                <span className={`text-[10px] font-black font-mono px-2 py-0.5 rounded-lg border ${abcClass === 'A' ? 'bg-success/10 text-success border-success/20' : abcClass === 'B' ? 'bg-primary/10 text-primary border-primary/20' : 'bg-gray-500/10 text-gray-500 border-gray-500/20'}`}>
                                    تصنيف {abcClass}
                                </span>
                            </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {/* Selling Price */}
                    <div className="bg-primary/5 p-4 rounded-2xl border border-primary/20 group hover:border-primary/40 transition-all">
                    ...
                    {/* Cost Price */}
                    <div className="bg-surface p-4 rounded-2xl border border-border group hover:border-primary/30 transition-all">
                        <div className="flex items-center gap-2 mb-2">
                    ...
                    {/* Profit */}
                    <div className="bg-surface p-4 rounded-2xl border border-border group hover:border-success/30 transition-all">
                        <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="font-mono font-bold text-success text-xl">{formatCurrency(profitAmount, prefs.currency).replace(prefs.currency, '')}</p>
                        <p className={`text-[10px] mt-1 font-bold ${profitMargin >= 20 ? 'text-success' : profitMargin >= 10 ? 'text-warning' : 'text-danger'}`}>
                    ...
                    <div className="bg-surface p-4 rounded-2xl border border-border group hover:border-primary/30 transition-all">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${scannedProduct.stock === 0 ? 'bg-danger/10' : scannedProduct.stock <= (scannedProduct.minStock || 5) ? 'bg-warning/10' : 'bg-primary/10'}`}>
                                    <Package size={14} className={scannedProduct.stock === 0 ? 'text-danger' : scannedProduct.stock <= (scannedProduct.minStock || 5) ? 'text-warning' : 'text-primary'} />
                                </div>
                                <p className="text-[10px] text-text-muted font-bold uppercase">المخزون</p>
                            </div>
                        </div>
                        <div className="flex items-center justify-between">
                            <p className={`font-mono font-black text-2xl ${scannedProduct.stock === 0 ? 'text-danger' : scannedProduct.stock <= (scannedProduct.minStock || 5) ? 'text-warning' : 'text-text-main'}`}>
                            ...
                            <div className="flex items-center gap-1 bg-bg rounded-lg p-1 border border-border">
                                <button
                                    ...
                                    className="w-7 h-7 flex items-center justify-center text-text-muted hover:text-danger hover:bg-danger/10 rounded-md transition-all"
                                ...
                                <button
                                    ...
                                    className="w-7 h-7 flex items-center justify-center text-text-muted hover:text-success hover:bg-success/10 rounded-md transition-all"
                ...
                {/* Stock Value Info */}
                <div className="bg-success/5 p-4 rounded-2xl border border-success/20 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="font-mono font-black text-success text-lg">{formatCurrency(stockValue, prefs.currency).replace(prefs.currency, '')} <span className="text-xs font-normal">{prefs.currency}</span></p>
                        </div>
                    </div>
                    <div className="text-left">
                        <p className="text-[10px] text-text-muted font-bold">الربح المتوقع</p>
                        <p className="font-mono font-bold text-success">{formatCurrency(profitAmount * scannedProduct.stock, prefs.currency).replace(prefs.currency, '')}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    >
                        <div className="w-10 h-10 rounded-xl bg-bg group-hover:bg-primary/10 flex items-center justify-center transition-all">
                            <Copy size={18} className="group-hover:text-primary transition-colors" />
                        </div>
                        نسخ المنتج
                    </button>
                    ...
                    <button
                        ...
                        className="p-3 bg-surface hover:bg-danger/5 text-text-main border border-border rounded-xl font-bold text-xs flex flex-col items-center gap-2 transition-all hover:border-danger/30 group"
                    >
                        <div className="w-10 h-10 rounded-xl bg-bg group-hover:bg-danger/10 flex items-center justify-center transition-all">
                            <Trash2 size={18} className="group-hover:text-danger transition-colors" />
                        </div>
                        حذف المنتج
                    </button>
                </div>
                ...
                <button
                    ...
                    className="w-full bg-primary text-primary-fg font-black py-4 rounded-2xl hover:brightness-110 shadow-xl shadow-primary/30 active:scale-[0.98] transition-all text-sm flex items-center justify-center gap-3"
                >
